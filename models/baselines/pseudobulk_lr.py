...
# Pseudobulk + L1 LR (LASSO) baseline.
...
def _patient_pseudobulk(X, pat_ids, celltypes, patients, ct_name):
    """patient × gene matrix of mean expression among cells of one CT."""
    n_feat = X.shape[1]
...
def _fit_lr(X_tr, y_tr, X_va):
    # Fit L1-regularized LR (LASSO)
    sc = StandardScaler()
    ...
    try:
        ...
        return np.full(X_va.shape[0], 0.5)
...
def run(args):
    ...
    out_dir = Path(args.results_dir) / args.cohort / "pseudobulk_lr"
    out_dir.mkdir(parents=True, exist_ok=True)

    n_pats = len(patients)
    summaries = {}

    # Per-CT L1 LR (LASSO) on patient pseudobulk
    print(f"\n[Per-CT] Pseudobulk + L1 LR (LASSO, C={C})")
    for ct_name in ct_unique:
        ct_mask = celltypes == ct_name
        n_cells = int(ct_mask.sum())
        if n_cells < MIN_CELLS_PER_CT:
            print(f"  [{ct_name}] SKIP — only {n_cells} cells")
            continue
        ...
        if n_yes < 2 or n_no < 2:
            print(f"  [{ct_name}] SKIP — Yes={n_yes}, No={n_no}")
            continue

        ct_pb = _patient_pseudobulk(X, pat_ids, celltypes, patients, ct_name)
        oof = np.full(n_pats, np.nan, dtype=np.float32)
        cnt = np.zeros(n_pats, dtype=np.int32)

        for tr_idx, va_idx in folds:
            va_probs = _fit_lr(ct_pb[tr_idx], pat_arr[tr_idx], ct_pb[va_idx])
        ...
        print(f"  [{ct_name}]  AUC={auc:.4f}  BalAcc={ba:.4f}  MCC={mcc:.4f}")
        summaries[ct_name] = {
            "fold_aucs": [auc], "mean_fold_auc": auc, "std_fold_auc": 0.0,
            "fold_aps": [ap], "mean_fold_ap": ap,
            "balanced_accuracy": ba, "mcc": mcc, "prevalence": prev,
            "cv_mode": effective_cv_mode(len(patients)), "n_folds": len(folds),
            "n_cells": n_cells, "n_patients_yes": n_yes, "n_patients_no": n_no,
        }

    # Patient-level prediction
    print(f"\n[Patient-level] per-CT PCA({N_PCA}) + L1 LR concat")
    ...
    for tr_idx, va_idx in folds:
        ...
        va_probs = _fit_lr(X_tr, pat_arr[tr_idx], X_va)
        for vi, pi in enumerate(va_idx):
            if np.isnan(oof[pi]):
                oof[pi] = 0.0
            oof[pi] += va_probs[vi]
...
def main():
    ...
    args = ap.parse_args()
    prof_start("pseudobulk_lr", args.cohort)
    run(args)
    prof_stop()
