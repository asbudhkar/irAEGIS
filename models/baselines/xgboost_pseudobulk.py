...
def _fit_xgb(X_tr, y_tr, X_va):
    sc = StandardScaler()
    X_tr_s = sc.fit_transform(X_tr)
    X_va_s = sc.transform(X_va)
    n_yes = int((y_tr == 1).sum()); n_no = int((y_tr == 0).sum())
    spw = float(n_no) / max(n_yes, 1) 
    clf = XGBClassifier(
...
def run(args):
    ...
    out_dir = Path(args.results_dir) / args.cohort / "xgboost_pseudobulk"
    out_dir.mkdir(parents=True, exist_ok=True)

    n_pats = len(patients)
    print(f"\n[Patient-level] per-CT PCA({N_PCA}) concat + XGBoost "
          f"(depth={MAX_DEPTH}, lr={LEARNING_RATE}, "
          f"n_estimators={N_ESTIMATORS}) — {len(ct_unique)} CTs kept")
    ...
    for tr_idx, va_idx in folds:
        ...
        va_probs = _fit_xgb(X_tr, pat_arr[tr_idx], X_va)
...
def main():
    ...
    args = ap.parse_args()
    prof_start("xgboost_pseudobulk", args.cohort)
    run(args)
    prof_stop()
